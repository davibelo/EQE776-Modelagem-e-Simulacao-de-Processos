...
O sistema diferencial foi integrado no intervalo $0 \leq t \leq 20\ \mathrm{h}$ empregando o método de Runge--Kutta explícito de quarta/quinta ordem com passo adaptativo (\texttt{solve\_ivp} do SciPy, método padrão \texttt{RK45}), impondo passo máximo de $0{,}01\ \mathrm{h}$ (equivalente a $36\ \mathrm{s}$). A solução contínua obtida foi posteriormente amostrada de forma uniforme em 100 pontos ao longo do intervalo de simulação, gerando as trajetórias discretizadas $(t, h_1, h_2)$ registradas em arquivo auxiliar para rastreabilidade.

\lstinputlisting[caption={Script Python utilizado para a integração numérica da Questão 1.},label={lst:python-q1}]{solucao_questao1.py}
...
\subsection*{Resolução numérica}
O sistema diferencial foi integrado no intervalo $0 \leq t \leq 60\ \mathrm{min}$ empregando o método de Runge--Kutta explícito de quarta/quinta ordem com passo adaptativo (\texttt{solve\_ivp} do SciPy, método padrão \texttt{RK45}), impondo passo máximo de $0{,}01\ \mathrm{min}$ (equivalente a $0{,}6\ \mathrm{s}$). A solução contínua obtida foi posteriormente amostrada de forma uniforme em 100 pontos ao longo do intervalo de simulação, gerando as trajetórias discretizadas $(t, h, x_A, x_B, x_C)$ registradas em arquivo auxiliar para rastreabilidade.

\lstinputlisting[caption={Script Python utilizado para a integração numérica da Questão 2.},label={lst:python-q2}]{solucao_questao2.py}
...
\subsection*{Resolução numérica}
O sistema diferencial foi integrado no intervalo $0 \leq t \leq 15\ \mathrm{min}$ empregando o método de Runge--Kutta explícito de quarta/quinta ordem com passo adaptativo (\texttt{solve\_ivp} do SciPy, método padrão \texttt{RK45}), impondo passo máximo de $0{,}01\ \mathrm{min}$ (equivalente a $0{,}6\ \mathrm{s}$). A solução contínua obtida foi posteriormente amostrada de forma uniforme em 100 pontos ao longo do intervalo de simulação, gerando as trajetórias discretizadas $(t, T, C_A, C_B, C_C, C_D, X_A)$ registradas em arquivo auxiliar para rastreabilidade.
...
\end{align}
onde $C_{\text{alim}}$ é a concentração de alimentação do reator.

Considerando o nó de mistura entre a corrente fresca ($Q_F, C_F$) e a corrente de reciclo ($Q_{\text{rec}}, C(t,1)$), o balanço algébrico fornece a vazão molar (ou mássica) de reagente que entra no reator,
\begin{equation}
  \dot{n}_A^{\text{mix}} = Q_F C_F + Q_{\text{rec}} C(t,1),
\end{equation}
e a vazão total que sai do misturador é
\begin{equation}
  \dot{n}^{\text{mix}} = Q_F + Q_{\text{rec}}.
\end{equation}
Assim, a concentração efetiva que alimenta o reator é a razão entre a vazão de reagente e a vazão total,
\begin{equation}
  C_{\text{alim}} = \frac{\dot{n}_A^{\text{mix}}}{\dot{n}^{\text{mix}}} = \frac{Q_F C_F + Q_{\text{rec}} C(t,1)}{Q_F + Q_{\text{rec}}}.
\end{equation}
Substituindo $Q_{\text{rec}} = R Q_F$, obtém-se a expressão utilizada:
\begin{equation}
...
\end{equation}
Para implementar numericamente as derivadas no primeiro nó físico, introduz-se um ponto fantasma imediatamente a montante de $z=0$, representado por $C_{\text{left}}$, que assegura a aplicação direta da condição de contorno mista no esquema de diferenças finitas. Utilizando diferenças avançadas, obtém-se a equação evolutiva
\begin{equation}
...
\end{equation}
Note que $C_0$ permanece associado ao nó físico de entrada da malha, enquanto $C_{\text{left}}$ representa um ponto fantasma fora do domínio calculado, introduzido apenas para que a condição de contorno mista em $z=0$ seja satisfeita de modo consistente com as aproximações por diferenças finitas das derivadas.
...
\end{equation*}
Introduzindo um ponto fantasma $C_{\text{right}}$ imediatamente a jusante de $z=1$, satisfazendo $C_{\text{right}} = C_{N-1}$, obtém-se a equação de evolução para o último nó físico (aqui denotado por $C_N$):
\begin{equation}
  \frac{dC_N}{dt} = -\frac{C_N - C_{N-1}}{\Delta z} + \frac{1}{Pe}\,\frac{C_{\text{right}} - 2C_N + C_{N-1}}{(\Delta z)^2} - Da\,C_N. \label{eq:dcn-q4}
\end{equation}
...
\subsection*{Resolução numérica}
O sistema de EDOs foi integrado no intervalo $0 \leq t \leq 5$ (unidades de tempo adimensionais) empregando o método de Runge--Kutta explícito de quarta/quinta ordem com passo adaptativo (\texttt{solve\_ivp} do SciPy, método padrão \texttt{RK45}), impondo passo máximo de $0{,}01$. A cada instante de tempo, a concentração de alimentação $C_{\text{alim}}$ é recalculada segundo \eqref{eq:calim-q4}, utilizando a concentração na posição de saída do reator $C(t,1)$. A solução contínua obtida foi posteriormente amostrada de forma uniforme em 100 pontos ao longo do intervalo de simulação, gerando as trajetórias discretizadas $(t, C_{\text{alim}}, C_{z=0{,}25}, C_{z=0{,}50}, C_{z=0{,}75}, C_{z=1})$ registradas em arquivo auxiliar para rastreabilidade.
